...
// PUT /sedes/:id - modifica una sede
export async function modificarSede(req, res) {
  try {
    ...
    if (!nombre || !direccion || !telefono) {
      return respuestaError(res, 400, "Nombre, dirección y teléfono son obligatorios");
    }

    const [existentes] = await pool.query("SELECT id FROM sede WHERE id = ?", [id]);
...
// DELETE /sedes/:id - no permite borrar sedes con usuarios o agenda asociada
export async function eliminarSede(req, res) {
  try {
    ...
    if (usuarios.length > 0 || agendas.length > 0) {
      return respuestaError(
        res,
        409,
        "No se puede eliminar la sede porque tiene usuarios o agenda asociados"
      );
    }
